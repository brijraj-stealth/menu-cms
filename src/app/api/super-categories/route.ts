...
export async function POST(request: Request) {
  ...
  try {
    ...
    const superCategory = await prisma.superCategory.create({ data: { ...data, menuId } });
    ...
    await prisma.activityLog.create({
      data: {
        ...
        entityId: superCategory.id,
        metadata: { entityName: superCategory.name },
      },
    });

    return Response.json({ data: superCategory }, { status: 201 });
  } catch {
    return Response.json({ error: "Failed to create super category" }, { status: 500 });
  }
}
