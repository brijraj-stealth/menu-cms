...
export async function POST(request: Request) {
  ...
  try {
    ...
    const textBlock = await prisma.textBlock.create({ data: { ...data, menuId } });
    return Response.json({ data: textBlock }, { status: 201 });
  } catch {
    return Response.json({ error: "Failed to create text block" }, { status: 500 });
  }
}
